...
const Project1screen = () => {
    ...
    const loadProjectsFromStorage = async () => {
        try {
            ...
            if (storedProjects) {
                ...
                console.log('All projects loaded successfully:', JSON.parse(storedProjects)); // Log saat data berhasil dimuat
            } else {
            ...
            [
                ...
                },
                {
                    ...
                    onPress: async () => {
                        ...
                        const newSessions = sessions.filter(session => session.P_id !== projectToDelete.P_id);
                        const newLocations = locations.filter(location => location.P_id !== projectToDelete.P_id);
                        const newSensorDatas = sensorDatas.filter(sensorData => sensorData.P_id !== projectToDelete.P_id);
    ...
    return (
        <View style={{ backgroundColor: "white", flex: 1 }}>
            ...
            {projects.length > 0 ? (
                <ScrollView>
                    ...
                    {projects.map((project, index) => (
                        <TouchableOpacity key={index} style={styles.box} onPress={() => handleProjectClick(project)}>
                            <View style={styles.projectInfo}>
                                <View>
                                    ...
                                    <View style={styles.dateContainer}>
                                        <Text style={styles.text3}>Start: {project.projectStart}</Text>
            ...
            <TouchableOpacity style={styles.circle} onPress={() => navigation.navigate('Createprojectscreen')}>
