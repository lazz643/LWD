...
import { View, Text, StyleSheet, TextInput, TouchableOpacity, ScrollView, Image, PermissionsAndroid, Platform, Alert } from 'react-native';
...
const Createlocationscreen = () => {
    ...
    const handleCreatePress = async () => {
        if (!L_title || !description || !soil || !projectStart || !latitude || !longitude) {
            Alert.alert('Alert', 'Please fill in all fields');
            return;
        }

        const newId = locations.length > 0 ? locations[locations.length - 1].L_id + 1 : 1;
        ...
        const isDuplicate = locations.some(
            (location) => location.L_title === L_title && location.P_id === P_id && location.S_id === S_id
        );
        /*
    ...
    return (
        <View style={{ backgroundColor: "white", flex: 1 }}>
            <Text style={styles.judul}>Create Location</Text>
