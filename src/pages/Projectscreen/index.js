...
class Projectscreen extends Component {
    ...
    componentDidMount() {
        ...
        this.focusListener = this.props.navigation.addListener('focus', () => {
            this.loadDeviceFromStorage();  // Fungsi ini akan dipanggil ulang ketika halaman aktif kembali
    ...
    processFFT = (amplitudeData, label) => {
        ...
        try {
            ...
            const frequencies = Array.from({ length: N }, (_, i) => i * (1 / 0.1) / N); // Assuming fs = 10 Hz
    ...
    findEModuli = (label) => {
        ...
        if (maxForce != null && settingData.poissonRatio != 0 && maxDeflection != null && settingData.diameter != 0) {
            const poissonRatioSquared = Math.pow(settingData.poissonRatio, 2);
            const radius = settingData.diameter / 2;
            const emoduli = (maxForce * (1 - poissonRatioSquared)) / (maxDeflection * radius);
    ...
    loadDeviceFromStorage = async () => {
        try {
            ...
            Alert.alert('Failed to load connected device from storage', error.message);
            this.setState({ connectedDevice: null });
        }
    };
    ...
    loadSensorDataFromStorage = async () => {
        ...
        try {
          ...
          if (storedSensorDatas) {
            ...
            const filteredSensorDatas = parsedSensorDatas.filter(sensordata => sensordata.L_id === location.L_id);
    ...
    handleSaveSensor = async () => {
        const { saveAllSensorData, location, sensortotal, sensorData, maxDeflection_s1, maxDeflection_s2, maxDeflection_s3, eModuli_s1, eModuli_s2, eModuli_s3, maxForce, dPath, dPath2, dPath3, dPath4, rOffset_1, rOffset_2, rOffset_3 } = this.state;
        if (!location || !sensortotal || !sensorData  || !maxForce ) {
          Alert.alert('Please fill in all fields');
          return;
        }
    
        const newId = saveAllSensorData.length > 0 ? saveAllSensorData[saveAllSensorData.length - 1].D_id + 1 : 1;
    ...
    saveSensorToStorage = async (updatedSensorData) => {
        try {
            await AsyncStorage.setItem('SensorDatas', JSON.stringify(updatedSensorData));
    ...
    handleIncomingData = (data) => {
        ...
        // Periksa apakah data null atau tidak valid

        if (this.state.isLoading) {
            // Cek apakah timer sudah aktif, jika belum baru jalankan setTimeout
            if (!data || data === 'null') {
                console.log("-------------------------------:");
                console.log(">>>>>>>>>>>>>>>>>> timer:", this.timer);
                if (!this.timer) {
                    // Set timer 3 detik jika isLoading = true dan data kosong
                ...
                return; // Lewati pemrosesan hingga timeout selesai atau data berubah
            }
            ...
            try {
                ...
                const stateUpdate = {
                    ...
                    receivedtime: parsedData.TimeData || [], // Handle missing or undefined TimeData
                    receivedforce: parsedData.ForceData || [], // Handle missing or undefined ForceData
                };
    
                // Process ForceData if necessary
                this.findMaxForce(parsedData.ForceData);
    
                // Check for the presence of AmplitudeData and update the state
                if (parsedData.AmplitudeData) {
                    this.setState({ rOffset_1: roffsetvalue });
                    this.processFFT(parsedData.AmplitudeData, 's1'); // Always process s1
                }
    
                if (parsedData.AmplitudeData2) {
                    this.setState({ rOffset_2: settingData.roffset });
                    this.processFFT(parsedData.AmplitudeData2, 's2'); // Process s2 if present
    
                    if (parsedData.AmplitudeData3) {
                        this.setState({ rOffset_3: settingData.roffset * 2 });
                        this.processFFT(parsedData.AmplitudeData3, 's3'); // Process s3 if present
                    }
                }
    
                // Update the state with the collected data
                this.setState(stateUpdate, () => {
                    this.valuegraph(); // Call valuegraph after updating the state
                });
    ...
    handleStartReading = async () => {
        ...
        if ( settingData.poissonRatio === 0 || settingData.diameter === 0 || settingData.roffset === 0) {
            this.setState({ isLoading: false });
            Alert.alert('Setting data values have not been set', 'Please go to settings to set the value.');
            return;
        }
        ...
        this.readInterval = setInterval(async () => {
            try {
                ...
                if (!stillConnected) {
                    ...
                    Alert.alert(`Disconnected with ${device.name}`, `Device ${device.name} is inactive or out of range`);
                } else {
                    let data = await device.read(); // Pastikan menggunakan connectedDevice, bukan device
    ...
    Masuk_Connectedscreen = async () => {
        ...
        if (stillOnBluetooth) {
            // Hentikan pembacaan data sebelum berpindah ke halaman lain
            if (this.readInterval) {
                clearInterval(this.readInterval);
    ...
    valuegraph = () => {
        const { receivedforce, receivedtime, amplitudeData_s1, amplitudeData_s2, amplitudeData_s3, yAxisLabels, yAxisLabels2, xAxisLabels, sensortotal } = this.state;
        ...
        if (maxValueAtYAxis) {
            receivedforce.forEach((val, index) => {
                ...
                const x_point = x_axis_x1_point + (x_value / maxValueAtXAxis * x_axis_actual_width);
                const y_point = y_axis_y2_point - (val / maxValueAtYAxis * y_axis_actual_height);
        ...
        const calculateDPath = (data, maxValueAtYAxis2) => {
            ...
            if (maxValueAtYAxis2) {
                data.forEach((val, index) => {
                    const x_value = receivedtime[index];
                    const x_point = x_axis_x1_point + (x_value / maxValueAtXAxis * x_axis_actual_width);
                    const y_point = y_axis_y1_point + (val / maxValueAtYAxis2 * y_axis_actual_height);
    ...
    renderGraph = (currentSensorData) => {
        ...
        const gap_between_x_axis_ticks = x_axis_actual_width / (xAxisLabels.length - 1);
    
        const y_axis_actual_height = y_axis_y2_point - y_axis_y1_point;
        const gap_between_y_axis_ticks = y_axis_actual_height / (yAxisLabels.length - 1);
        ...
        const render_x_axis_labels_and_ticks = xAxisLabels.map((item, index) => {
            ...
            return (
                <G key={`x-axis labels and ticks${index}`}>
                    <Line x1={x_point} y1={x_axis_y1_point} x2={x_point} y2={x_axis_y1_point + 5} strokeWidth={2} stroke={'black'} />
                    <Line x1={x_point} y1={x_axis_y1_point} x2={x_point} y2={y_axis_y1_point} strokeWidth={2} stroke={'grey'} />
                    <SvgText x={x_point} y={x_axis_y1_point + 18} fontWeight="400" fontSize={10} fill={'black'} textAnchor="middle">
        ...
        const render_y_axis_labels_and_ticks = yAxisLabels.map((item, index) => {
            ...
            return (
                <G key={`y-axis labels and ticks${index}`}>
                    <Line x1={marginFor_x_fromLeft} y1={y_point} x2={marginFor_x_fromLeft - 5} y2={y_point} stroke={'black'} strokeWidth={2} />
                    <Line x1={marginFor_x_fromLeft} y1={y_point} x2={x_axis_x2_point} y2={y_point} stroke={'grey'} strokeWidth={2} />
                    {(index === 0 || index === 5 || index === 10) && (
                        <SvgText x={marginFor_x_fromLeft - 10} y={y_point} fontWeight="400" fontSize={10} fill={'black'} textAnchor="end">
                            {item}
                        </SvgText>
                    )}
                    {(index === 6) && (
        ...
        const render_y_axis_labels_and_ticks2 = yAxisLabels2.map((item, index) => {
            ...
            return (
                <G key={`y-axis labels and ticks2${index}`}>
                    <Line x1={marginFor_x_fromLeft - 40} y1={y_point} x2={marginFor_x_fromLeft - 45} y2={y_point} stroke={'black'} strokeWidth={2} />
                    {(index === 0 || index === 5 || index === 10) && (
                        <SvgText x={marginFor_x_fromLeft - 50} y={y_point} fontWeight="400" fontSize={10} fill={'black'} textAnchor="end">
                            {item}
                        </SvgText>
                    )}
                    {(index === 4) && (
        ...
        return (
            <View style={[styles.svgWrapper, { height: 270 }]}>
                <Svg style={styles.svgStyle}>
                    ...
                    <G key="x-axis y-axis">
                        <Line x1={x_axis_x1_point} y1={x_axis_y1_point} x2={x_axis_x2_point} y2={x_axis_y2_point} stroke={'grey'} strokeWidth={2} />
                        <Line x1={y_axis_x1_point} y1={y_axis_y1_point} x2={y_axis_x2_point} y2={y_axis_y2_point} stroke={'grey'} strokeWidth={2} />
                    </G>
                    ...
                    {Array.isArray(saveSensorData) && saveSensorData.length > 0 && currentSensorData.dPath && <Path d={currentSensorData.dPath} strokeWidth={2} stroke={'red'} fill="none" />}
                    {Array.isArray(saveSensorData) && saveSensorData.length > 0 && currentSensorData.dPath2 && <Path d={currentSensorData.dPath2} strokeWidth={2} stroke={'blue'} fill="none" />}
                    {Array.isArray(saveSensorData) && saveSensorData.length > 0 && currentSensorData.dPath3 && <Path d={currentSensorData.dPath3} strokeWidth={2} stroke={'green'} fill="none" />}
                    {Array.isArray(saveSensorData) && saveSensorData.length > 0 && currentSensorData.dPath4 && <Path d={currentSensorData.dPath4} strokeWidth={2} stroke={'purple'} fill="none" />}
                </Svg>
                <View style={styles.labelContainer}>
                    {Array.isArray(saveSensorData) && saveSensorData.length > 0 && currentSensorData.sensortotal >= 1 && (
                        <>
                            <Text style={{ fontSize: 12, color: 'white', fontWeight: 'bold', margin: 5, backgroundColor: 'red', borderWidth: 1, borderRadius: 2, borderColor: 'red', paddingHorizontal: 5 }}>P</Text>
                            <Text style={{ fontSize: 12, color: 'white', fontWeight: 'bold', margin: 5, backgroundColor: 'blue', borderWidth: 1, borderRadius: 2, borderColor: 'blue', paddingHorizontal: 5 }}>D1</Text>
                        </>
                    )}
                    {Array.isArray(saveSensorData) && saveSensorData.length > 0 && currentSensorData.sensortotal === 3 && (
                        <>
                        <Text style={{ fontSize: 12, color: 'white', fontWeight: 'bold', margin: 5, backgroundColor: 'green', borderWidth: 1, borderRadius: 2, borderColor: 'green', paddingHorizontal: 5 }}>D2</Text>
                            <Text style={{ fontSize: 12, color: 'white', fontWeight: 'bold', margin: 5, backgroundColor: 'purple', borderWidth: 1, borderRadius: 2, borderColor: 'purple', paddingHorizontal: 5 }}>D3</Text>
                        </>
                    )}
                </View>
            </View>
    ...
    createExcelFile = async (option) => {
        ...
        const titleSetting = [
            "Target E", "Emoduli Value", "Correct Emoduli Value", "Target E1", "Target E2", "Top Layer Thickness", "Target D1", 
            "Deflection Value", "Target Pressure", "Delta Deflection Value", "Force", "Press", "Pulse", "Diameter", "Poisson Ratio", "R offset"
        ];
        const title = [
            "Drops", "Number Of Sensor", "Pulse", "Press", "Poisson Ratio", "Max Deflection Sensor 1", 
            "Max Deflection Sensor 2", "Max Deflection Sensor 3", "E-Moduli Sensor 1", "E-Moduli Sensor 2", 
            "E-Moduli Sensor 3", "rOffset Sensor 1", "rOffset Sensor 2", "rOffset Sensor 3", "Max Force"
        ];
    ...
    render() {
        const { totalData, isLoading, settingData, sensortotal, saveSensorData } = this.state;
        const buttonColor = this.state.connectedDevice ? '#4541E4' : 'grey';
        const buttonColor2 = Array.isArray(saveSensorData) && saveSensorData.length > 0 ? '#4541E4' : 'grey';
            ...
            <View style={{ backgroundColor: "white", flex: 1 }}>
                ...
                {this.state.connectedDevice && (
                    <>
                        ...
                        <View style={{flexDirection: 'row', alignItems: 'center', alignSelf: 'center'}}>
                            <Text style={{ fontSize: 13, color: 'grey', marginTop: 5, marginBottom: 10, fontWeight: 'bold' }}>Change Device? </Text>
                            <TouchableOpacity  style={styles.buttontobluetooth} onPress={this.Masuk_Connectedscreen}>
                                <Text style={{ fontSize: 13, color: '#4541E4', fontWeight: 'bold' }}>Click Here</Text>
                            </TouchableOpacity>
                        </View>
                    </>
                )}
                ...
                {!this.state.connectedDevice && (
                    <>
                        ...
                        <View style={{flexDirection: 'row', alignItems: 'center', alignSelf: 'center'}}>
                            <Text style={{ fontSize: 13, color: 'grey', marginTop: 5, marginBottom: 10, fontWeight: 'bold' }}>Make a connection? </Text>
                            <TouchableOpacity  style={styles.buttontobluetooth} onPress={this.Masuk_Connectedscreen}>
                                <Text style={{ fontSize: 13, color: '#4541E4', fontWeight: 'bold' }}>Click Here</Text>
                            </TouchableOpacity>
                        </View>
                ...
                <ScrollView>
                    ...
                    <View style={styles.sensorSelectionContainer}>
                        ...
                        <TouchableOpacity
                            ...
                            onPress={() => {
                                if (saveSensorData.length > 0) {
                                    this.setState((prevState) => ({
                                        totalData: Math.min(prevState.totalData + 1, saveSensorData.length), // Mencegah melebihi panjang array
                                    }));
                                }
                            }}
                            disabled={saveSensorData.length === 0}
                    ...
                    <View>
                        {this.renderGraph(currentSensorData)}
                    </View>
                    ...
                    <View style={styles.geophoneContainer}>
                        ...
                        <View style={styles.dataItem}>
                            ...
                            <Text style={styles.value}>{currentSensorData.maxForce || 0}</Text>
                        </View>
                        <View style={styles.dataItem}>
                        ...
                        {Array.isArray(saveSensorData) && saveSensorData.length > 0 && currentSensorData.sensortotal >= 1 && 
                            this.renderGeophoneSensor(1, currentSensorData.rOffset_1, currentSensorData.maxDeflection_s1, currentSensorData.eModuli_s1)
                        }
                        {Array.isArray(saveSensorData) && saveSensorData.length > 0 && currentSensorData.sensortotal >= 2 && 
                            this.renderGeophoneSensor(2, currentSensorData.rOffset_2, currentSensorData.maxDeflection_s2, currentSensorData.eModuli_s2)
                        }
                        {Array.isArray(saveSensorData) && saveSensorData.length > 0 && currentSensorData.sensortotal >= 3 && 
                            this.renderGeophoneSensor(3, currentSensorData.rOffset_3, currentSensorData.maxDeflection_s3, currentSensorData.eModuli_s3)
                        }
                    ...
                    <View style={styles.geophoneContainer}>
                        ...
                        <View style={styles.dataItem}>
                            ...
                            <Text style={styles.value}>{settingData?.poissonRatio ?? "-"}</Text>
                        </View>

                        <TouchableOpacity style={styles.box4} onPress={this.Masuk_Settingscreen}>
                ...
                {isLoading && (
                    ...
                    <View style={styles.loadingContainer}>
                        ...
                        <Text style={{color: '#4541E4', marginTop: 10, alignSelf: 'center'}}>Loading...</Text>
                    </View>
                    </View>
                )}
            </View>
...
const styles = StyleSheet.create({
    ...
    sensorSelectionContainer: {
        flexDirection: 'row',
        justifyContent: 'space-between', // Mengatur jarak antara tombol
        alignItems: 'center',
        marginHorizontal: 10,
        marginTop: 10,
    },
    indexText: {
        ...
        fontWeight: 'bold',
        flex: 2, // Mengizinkan teks mengisi ruang di antara tombol
    },
    navigationButton: {
        flex: 1, // Membuat tombol mengisi ruang secara proporsional
        alignItems: 'center',
        justifyContent: 'center',
        padding: 7,
        backgroundColor: '#4541E4', // Ganti dengan warna yang diinginkan
        borderRadius: 5,
    ...
    sensorButtonsContainer: {
        flexDirection: 'row',       // Mengatur agar item berbaris secara horizontal
        justifyContent: 'space-between',  // Mengatur jarak antara tombol
        marginTop: 14,
        marginHorizontal: 16,
    },
    ...
    sensorButton: {                  
        ...
        borderColor: '#4541E4',
        borderWidth: 2,
        borderRadius: 5,       // Memberikan sedikit jarak horizontal antar tombol
        alignItems: 'center',
        marginBottom: 6,
    },
    sensorButtonText: {
